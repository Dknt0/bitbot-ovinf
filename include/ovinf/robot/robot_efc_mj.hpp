...
#include "efc_mj_common.h"
#include "utils/csv_logger.hpp"

namespace ovinf {

class RobotEfcMj : public RobotBase<float> {
  using VectorT = Eigen::Matrix<float, Eigen::Dynamic, 1>;
  friend class ObserverEfcMj;
  friend class ExecutorEfcMj;

 public:
  using Ptr = std::shared_ptr<RobotEfcMj>;

 private:
  class ObserverEfcMj : public ObserverBase {
   public:
    ObserverEfcMj() = delete;
    ObserverEfcMj(RobotBase<float>* robot, const YAML::Node& config)
    ...
    virtual bool Update() final {
      auto robot_mj = dynamic_cast<RobotEfcMj*>(robot_);
  ...
  class ExecutorEfcMj : public ExecutorBase {
   public:
    ExecutorEfcMj() = delete;
    ExecutorEfcMj(RobotBase<float>* robot, const YAML::Node& config)
 ...
 public:
  RobotEfcMj() = delete;
  RobotEfcMj(const YAML::Node& config) : RobotBase(config) {
    motors_.resize(motor_size_);
    this->observer_ = std::make_shared<ObserverEfcMj>((RobotBase<float>*)this,
                                                       config["observer"]);
    this->executor_ = std::make_shared<ExecutorEfcMj>((RobotBase<float>*)this,
                                                       config["executor"]);
  }
...
void RobotEfcMj::GetDevice(const KernelBus& bus) {
  for (size_t i = 0; i < motor_size_; ++i) {
...
void RobotEfcMj::ObserverEfcMj::WriteLog() {
