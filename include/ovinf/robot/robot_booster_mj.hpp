#ifndef ROBOT_BOOSTER_MJ_HPP
#define ROBOT_BOOSTER_MJ_HPP
...
#include "booster_mj_common.h"
#include "utils/csv_logger.hpp"

namespace ovinf {

class RobotBoosterMj : public RobotBase<float> {
  using VectorT = Eigen::Matrix<float, Eigen::Dynamic, 1>;
  friend class ObserverBoosterMj;
  friend class ExecutorBoosterMj;

 public:
  using Ptr = std::shared_ptr<RobotBoosterMj>;

 private:
  class ObserverBoosterMj : public ObserverBase {
   public:
    ObserverBoosterMj() = delete;
    ObserverBoosterMj(RobotBase<float>* robot, const YAML::Node& config)
    ...
    virtual bool Update() final {
      auto robot_mj = dynamic_cast<RobotBoosterMj*>(robot_);
  ...
  class ExecutorBoosterMj : public ExecutorBase {
   public:
    ExecutorBoosterMj() = delete;
    ExecutorBoosterMj(RobotBase<float>* robot, const YAML::Node& config)
 ...
 public:
  RobotBoosterMj() = delete;
  RobotBoosterMj(const YAML::Node& config) : RobotBase(config) {
    motors_.resize(motor_size_);
    this->observer_ = std::make_shared<ObserverBoosterMj>((RobotBase<float>*)this,
                                                       config["observer"]);
    this->executor_ = std::make_shared<ExecutorBoosterMj>((RobotBase<float>*)this,
                                                       config["executor"]);
  }
...
void RobotBoosterMj::GetDevice(const KernelBus& bus) {
  for (size_t i = 0; i < motor_size_; ++i) {
...
void RobotBoosterMj::ObserverBoosterMj::CreateLog(YAML::Node const& config) {
...
void RobotBoosterMj::ObserverBoosterMj::WriteLog() {
